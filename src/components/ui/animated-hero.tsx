...
export const ParticleHero = React.forwardRef<HTMLElement, ParticleHeroProps>(function ParticleHero(
  {
    id,
    title = "FLUX",
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < totalParticles; i++) {
      ...
      particle.style.cssText = `
        ...
        opacity: ${opacity};
        background: hsl(243, 100%, ${lightness}%);
        box-shadow: 0 0 ${glowSize * 0.2}rem 0 hsl(243, 100%, 71%);
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 min-h-screen flex flex-col items-center justify-center px-6">
        {children ? (
          ...
          <div className="text-center max-w-6xl mx-auto">
            {/* Main Title */}
            <div className="mb-16">
              <h1 className="text-8xl md:text-[10rem] lg:text-[14rem] xl:text-[16rem] font-semibold tracking-tighter leading-[0.8] mb-8 text-white">
                {title}
              </h1>

              {/* Subtitle */}
              <div className="space-y-4">
                <h2 className="font-mono text-2xl md:text-4xl lg:text-5xl font-normal text-text-muted tracking-[0.2em] uppercase">
                  {subtitle}
                </h2>
            ...
            {description && (
              <div className="mb-20">
                <p className="text-lg md:text-xl lg:text-2xl text-text-muted font-light max-w-3xl mx-auto leading-relaxed">
                  {description}
                </p>
              </div>
            )}
            ...
            <div className="space-y-8">
              <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
                {primaryButton && (
                  <button onClick={primaryButton.onClick} className="btn-primary">
                    {primaryButton.text}
                  </button>
                )}

                {secondaryButton && (
                  <button onClick={secondaryButton.onClick} className="btn-secondary">
              ...
              {/* Interactive hint */}
              {interactiveHint && (
                <div className="flex items-center justify-center gap-6 font-mono text-text-faint text-sm uppercase tracking-[0.3em]">
                  <div className="w-12 h-px bg-gradient-to-r from-transparent to-[var(--accent-line)]"></div>
                  <span className="animate-pulse">{interactiveHint}</span>
                  <div className="w-12 h-px bg-gradient-to-l from-transparent to-[var(--accent-line)]"></div>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-20 left-20 w-80 h-80 bg-accent/5 rounded-full blur-3xl animate-pulse"></div>
        <div
          className="absolute bottom-20 right-20 w-96 h-96 bg-accent/5 rounded-full blur-3xl animate-pulse"
          style={{ animationDelay: "2s" }}
        ></div>
        <div
          className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[120vh] h-[120vh] rounded-full"
          style={{
            background:
